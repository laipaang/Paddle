...
class BasicAucCalculator {
 public:
  ...
  void add_unlock_data(double pred, int label, float sample_scale=1.0f);
  // add batch data
...
class BoxWrapper {
 ...
 public:
  ...
  class MetricMsg {
   public:
    ...
    MetricMsg(const std::string& label_varname, const std::string& pred_varname,
              ...
              bool mode_collect_in_gpu = false, int max_batch_size = 0, const std::string& sample_scale_varname="")
      ...
      if (!sample_scale_varname_.empty()) {
        ...
        PADDLE_ENFORCE_EQ(
            ...
            platform::errors::PreconditionNotMet(
                ...
                label_len, sample_scale_data.size()));
        calculator->add_sample_data(pred_data, label_data, sample_scale_data, label_len, place);
      } else {
  ...
  void InitMetric(const std::string& method, const std::string& name,
                  ...
                  int max_batch_size = 0, const std::string& sample_scale_varname="") {
    if (method == "AucCalculator") {
      metric_lists_.emplace(
          name,
          new MetricMsg(label_varname, pred_varname, metric_phase, bucket_size,
                        mode_collect_in_gpu, max_batch_size, sample_scale_varname));
